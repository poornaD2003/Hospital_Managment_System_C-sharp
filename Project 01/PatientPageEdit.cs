    ...
    public partial class PatientPageEdit : Form
    {
        ...
        private void LoadPatientData()
        {
            try
            {
                ...
                {
                    string query = "SELECT patientID, email, phoneNumber, address, age, sex, bloodGroup, userName, password, patientImage FROM patient"; using (SqlDataAdapter adapter = new SqlDataAdapter(query, conn))
                    {
                        DataTable dt = new DataTable();
                        adapter.Fill(dt);
                        dataGridView1.DataSource = dt;

                        // Hide password and image columns from display grids to remain professional
                        if (dataGridView1.Columns.Contains("password"))
                            dataGridView1.Columns["password"].Visible = false;

                        if (dataGridView1.Columns.Contains("patientImage"))
                            dataGridView1.Columns["patientImage"].Visible = false;
                    }
                }
            }
            catch (Exception ex)
                ...
                using (SqlConnection conn = dbConnection.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        if (picPatientImage.Image != null)
                        {
                            ...
                            {
                                using (Bitmap bmp = new Bitmap(picPatientImage.Image))
                                {
